    ...
    <div className="max-w-6xl mx-auto w-full animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {courses.length === 0 ? (
          ...
          courses.map((course) => (
            <Link key={course.id} href={`/dashboard/course/${course.id}`} className="bg-white dark:bg-army-800 rounded-3xl shadow-lg border border-gray-100 dark:border-army-700 hover:-translate-y-2 hover:shadow-2xl transition duration-300 flex flex-col overflow-hidden group">
              <div className="h-48 overflow-hidden relative">
                <span className="absolute top-4 left-4 bg-red-600 text-white text-xs font-black px-3 py-1.5 rounded-lg shadow-lg z-10 border border-red-400/50">
              ...
              <div className="p-6 flex-1 flex flex-col relative">
                <div className="absolute -top-5 right-5 bg-white dark:bg-army-800 p-1.5 rounded-xl shadow-sm">
                  <span className="text-xs font-black text-rta-DEFAULT dark:text-rta-accent bg-[#f1f4e6] dark:bg-army-700 px-3 py-1 rounded-lg border border-rta-DEFAULT/20 dark:border-army-600">R-POST</span>
                </div>
